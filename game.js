...
const flipGameScreen = document.getElementById('flip-game-screen');
const boxesGameScreen = document.getElementById('boxes-game-screen');
...
class InteractivePet {
    ...
    setupEvents() {
        const handleInteraction = (e, isClick = false) => {
            ...
            this.handlePetInteraction(x, y, isClick);
        };

        this.canvas.addEventListener('mousemove', (e) => handleInteraction(e));
        this.canvas.addEventListener('touchstart', (e) => {
            e.preventDefault();
            handleInteraction(e, true);
        }, { passive: false });
        this.canvas.addEventListener('mousedown', (e) => handleInteraction(e, true));
    }

    handlePetInteraction(x, y, isClick = false) {
        if (isClick && speedPettingManager.isActive) {
            speedPettingManager.registerClick();
            this.purrIntensity = 0.8;
            this.lastInteraction = Date.now();
            return;
        }
        ...
        if (dist < this.headRegion.r) {
            this.mood = Math.min(100, this.mood + (isClick ? 2 : 0.5));
            this.purrIntensity = Math.min(1, this.purrIntensity + 0.1);
            this.lastInteraction = Date.now();
            if (isClick && this.mood > 95) {
...
window.openFlipGame = () => {
    arcadeScreen.classList.add('hidden');
    flipGameScreen.classList.remove('hidden');
};

window.closeFlipGame = () => {
    flipGameScreen.classList.add('hidden');
    arcadeScreen.classList.remove('hidden');
};

let isFlipping = false;
window.playFlip = (prediction) => {
    if (isFlipping) return;
    if (economyData.atoms < 50) {
        showToast("YETERSİZ ATOM", "Bahis için 50 ⚛ gerekiyor.", "fa-triangle-exclamation");
        return;
    }

    economyData.atoms -= 50;
    saveEconomy();
    updateShopUI();
    isFlipping = true;

    coinEl.classList.add('spinning');
    coinEl.classList.remove('coin-land-alive', 'coin-land-dead');

    setTimeout(() => {
        const isAlive = Math.random() > 0.5;
        const result = isAlive ? 'ALIVE' : 'DEAD';

        coinEl.classList.remove('spinning');
        coinEl.classList.add(isAlive ? 'coin-land-alive' : 'coin-land-dead');

        setTimeout(() => {
            if (prediction === result) {
                const winAmount = 100;
                economyData.atoms += winAmount;
                showToast("KAZANDIN!", `Kuantum durumu doğrulandı: ${winAmount} ⚛`, "fa-check-circle");
            } else {
                showToast("KAYBETTİN", "Kuantum çöküşü beklendiği gibi gitmedi.", "fa-xmark-circle");
            }
            saveEconomy();
            updateShopUI();
            isFlipping = false;
...
// 2. QUANTUM BOXES
window.openBoxesGame = () => {
    arcadeScreen.classList.add('hidden');
    boxesGameScreen.classList.remove('hidden');
    resetBoxes();
};

window.closeBoxesGame = () => {
    boxesGameScreen.classList.add('hidden');
    arcadeScreen.classList.remove('hidden');
};

let boxesData = [];
let canSelectBox = true;

function resetBoxes() {
    canSelectBox = true;
    boxesData = ['empty', 'poison', 'reward'].sort(() => Math.random() - 0.5);
    document.querySelectorAll('.quantum-box').forEach(box => {
        box.classList.remove('open', 'winner', 'loser');
    });
}

window.selectBox = (index) => {
    if (!canSelectBox) return;
    if (economyData.atoms < 30) {
        showToast("YETERSİZ ATOM", "Giriş için 30 ⚛ gerekiyor.", "fa-triangle-exclamation");
        return;
    }

    economyData.atoms -= 30;
    saveEconomy();
    updateShopUI();
    canSelectBox = false;

    const result = boxesData[index];
    const boxes = document.querySelectorAll('.quantum-box');
    ...
    setTimeout(() => {
        if (result === 'reward') {
            const amount = 150;
            economyData.atoms += amount;
            selectedBox.classList.add('winner');
            showToast("BÜYÜK ÖDÜL!", `Kutudan ${amount} ⚛ çıktı!`, "fa-gift");
        } else if (result === 'poison') {
            selectedBox.classList.add('loser');
            showToast("ZEHİRLİ GAZ!", "Hiçbir şey bulamadın.", "fa-biohazard");
        } else {
            showToast("BOŞLUK", "Kutu tamamen boş çıktı.", "fa-circle-question");
        }
...
// 3. SPEED PETTING MANAGER
const speedPettingManager = {
    isActive: false,
    clicks: 0,
    targetClicks: 15,
    timer: null,
    ...
    init() {
        // Her 2-5 dakikada bir tetikle
        this.triggerInterval = setInterval(() => {
            if (!isPlaying && !this.isActive && Math.random() > 0.7) {
                this.startEvent();
            }
        }, 60000);
    },
    ...
    startEvent() {
        if (isArcadeOpen || isFlipping) return;
        this.isActive = true;
        this.clicks = 0;
        petZone.classList.add('quantum-glow');
        petTooltip.innerHTML = '<i class="fa-solid fa-bolt"></i> HIZLI SEV! (15 tık)';
    ...
    endEvent(success) {
        ...
        petZone.classList.remove('quantum-glow');
        petTooltip.innerHTML = '<i class="fa-solid fa-hand-pointer"></i> Bana dokun!';

        if (success) {
            const reward = 50;
            economyData.atoms += reward;
            saveEconomy();
            updateShopUI();
            showToast("MÜKEMMEL!", `Kedi mutlu oldu: +${reward} ⚛`, "fa-heart");
        }
    }
};

speedPettingManager.init();

// 4. KUANTUM GÖZLEMİ (STEALTH GAME)
const stealthGameScreen = document.getElementById('stealth-game-screen');
const quantumEye = document.getElementById('quantum-eye');
const stealthProgress = document.getElementById('stealth-progress');
const stealthCatIcon = document.getElementById('stealth-cat-icon');
const stealthControl = document.getElementById('stealth-control');
const observationWarning = document.getElementById('observation-warning');

let isStealthActive = false;
let isHoldingDown = false;
let stealthDistance = 0;
const targetDistance = 1000;
let eyeState = 'closed'; // 'closed', 'opening', 'open'
let eyeTimer = 0;
let stealthGameOver = false;

window.openStealthGame = () => {
    arcadeScreen.classList.add('hidden');
    stealthGameScreen.classList.remove('hidden');
    resetStealthGame();
    isStealthActive = true;
    requestAnimationFrame(stealthLoop);
};

window.closeStealthGame = () => {
    isStealthActive = false;
    stealthGameScreen.classList.add('hidden');
    arcadeScreen.classList.remove('hidden');
};

function resetStealthGame() {
    stealthDistance = 0;
    stealthGameOver = false;
    isHoldingDown = false;
    eyeState = 'closed';
    eyeTimer = Math.random() * 2000 + 1000;
    updateStealthUI();
    quantumEye.className = 'quantum-eye closed';
    observationWarning.style.opacity = '0';
}

function updateStealthUI() {
    const percent = (stealthDistance / targetDistance) * 100;
    stealthProgress.style.width = percent + '%';
    stealthCatIcon.style.left = percent + '%';
}

// Kontroller
stealthControl.addEventListener('pointerdown', (e) => {
    e.preventDefault();
    isHoldingDown = true;
    stealthControl.textContent = "DURUYORSUN...";
});

window.addEventListener('pointerup', () => {
    isHoldingDown = false;
    if (isStealthActive) {
        stealthControl.textContent = "KOŞUYORSUN!";
    }
});

function stealthLoop(timestamp) {
    if (!isStealthActive || stealthGameOver) return;

    // Göz Mantığı
    eyeTimer -= 16; // Yaklaşık 60fps
    if (eyeTimer <= 0) {
        if (eyeState === 'closed') {
            eyeState = 'opening';
            quantumEye.className = 'quantum-eye open blinking';
            audioManager.playSwitch(); // Göz açılma uyarısı
            eyeTimer = 500;
        } else if (eyeState === 'opening') {
            eyeState = 'open';
            quantumEye.className = 'quantum-eye open observing';
            eyeTimer = Math.random() * 2000 + 1000;
        } else {
            eyeState = 'closed';
            quantumEye.className = 'quantum-eye closed';
            audioManager.playSwitch(); // Göz kapanma
            eyeTimer = Math.random() * 3000 + 1500;
        }
    }

    // Hareket ve Yakalanma
    if (!isHoldingDown) {
        // El çekiliyse (koşuyor)
        if (eyeState === 'open') {
            // Göz açıkken koşarsa YAKALANDI
            handleStealthFailure();
        } else {
            stealthDistance += 2;
            if (stealthDistance >= targetDistance) {
...
function handleStealthFailure() {
    stealthGameOver = true;
    observationWarning.style.opacity = '1';
    showToast("YAKALANDIN!", "Gözlemci seni fark etti.", "fa-eye-slash");
    audioManager.playGameOver(); // Yakalanma sesi
...
function handleStealthSuccess() {
    stealthGameOver = true;
    const reward = 200;
    economyData.atoms += reward;
    saveEconomy();
    updateShopUI();
    showToast("BAŞARDIN!", `Hedefe ulaştın: +${reward} ⚛`, "fa-flag-checkered");
